...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Widget build(BuildContext context) =>
      ListView(padding: const EdgeInsets.only(top: 8, bottom: 30), children: [
        _title('Lista attiva'),
        SurfaceCard(
            child: Padding(
        ...
        SurfaceCard(
          ...
          subtitle: Text(widget.user.ruolo),
        )),
        SurfaceCard(
          ...
          onChanged:
              busy ? null : (enabled) => enabled ? enable2fa() : disable2fa(),
        )),
  ...
  Future<void> deleteList() async {
    if (!await confirmDialog(context, 'Eliminare definitivamente la lista?',
  ...
  Future<void> enable2fa() async {
    await mutate(() async {
      ...
      final bytes =
          base64Decode((setup['qr_data_url'] as String).split(',').last);
  ...
  Future<void> createUser() async {
    ...
    if (data == null) return;
    await mutate(() async {
      await widget.api.createUser(data.$1, data.$2!, data.$3);
      await loadRestricted();
  ...
  Future<void> editUser(AdminUser user) async {
    ...
    if (data == null) return;
    await mutate(() async {
      await widget.api.updateUser(user.id,
          role: user.username == 'amos' ? null : data.$3, password: data.$2);
      await loadRestricted();
...
class InviteTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SurfaceCard(
        child: ListTile(
          ...
          title: const Text('Invita partecipante'),
          subtitle: Text(availableCount == null
              ? 'Caricamento…'
              : availableCount == 0
...
class AdminSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      Column(key: const Key('admin-section'), children: [
        if (users == null) const CircularProgressIndicator(),
        ...?users?.map((user) => SurfaceCard(
                child: ListTile(
              title: Text(user.username),
              subtitle:
                  Text('${user.ruolo}${user.totpEnabled ? ' · 2FA' : ''}'),
              trailing: IconButton(
...
Future<(String, String?, String)?> userDialog(BuildContext context,
  ...
  final result = await showDialog<(String, String?, String)>(
      ...
      builder: (dialogContext) => StatefulBuilder(
          builder: (context, setState) => AlertDialog(
                title: Text(
                    user == null ? 'Crea utente' : 'Modifica ${user.username}'),
                content: SingleChildScrollView(
                    child: Column(mainAxisSize: MainAxisSize.min, children: [
                  TextField(
                      controller: username,
                  ...
                  if (user?.username != 'amos') ...[
                    ...
                    DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'guida', child: Text('Guida')),
                          DropdownMenuItem(
                              value: 'follower', child: Text('Follower'))
                        ],
                  ...
                  FilledButton(
                      onPressed: () {
                        ...
                        if (username.text.trim().isEmpty) {
                          setState(() => error = 'Inserisci lo username');
                          return;
                        }
